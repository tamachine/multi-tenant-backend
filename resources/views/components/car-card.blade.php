<div class="rounded-2lg flex flex-col git push bg-white">
    <div class="bg-[#f4f5f6] rounded-t-2lg">
        <div class="w-80 h-48 text-gray-600 group flex justify-center {{ $noImages ? 'items-end' : 'items-center' }} relative">
            ...
            <div class="absolute bottom-5 right-5 bg-white px-2 py-1 rounded-md border border-[#E7ECF3] text-xs cursor-pointer hover:bg-black hover:text-white hover:border-pink-red group">
                <span>{{ __('cars.card-quick-look') }}</span> <img class="inline group-hover:hidden" src="{{ asset('images/icons/eye.svg') }}" /> <img class="hidden group-hover:inline" src="{{ asset('images/icons/eye-white.svg') }}" />
            </div>
        </div>
    </div>
